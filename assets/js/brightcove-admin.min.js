...
_.each(wpbc.requests,function(a){a.abort()}),wpbc.requests=[]},checksum:function(a){_.isString(a)||(a=_.isFunction(a.toJSON)?a.toJSON():JSON.stringify(a));for(var b=305419896,c=0;c<a.length;c++)b+=a.charCodeAt(c)*(c+1);return b},/**
		 ...
		 * @returns {Promise}
		 */
sync:function(a,b,c){var d;
// Overload the read method so Media.fetch() functions correctly.
if("read"===a){c=c||{},c.data=_.extend(c.data||{},{action:"bc_media_query",account:this.activeAccount||wpbc.preload.defaultAccountId,dates:this.date,posts_per_page:wpbc.posts_per_page,page_number:this.pageNumber,nonce:wpbc.preload.nonce,search:this.searchTerm,tags:this.tag,tagName:wpbc.preload.tags[this.tag],type:this.mediaType||"videos"});var e=_.pick(c.data,"account","dates","posts_per_page","search","tags","type");
// Determine if we're infinite scrolling or not.
this.additionalRequest=_.isEqual(e,wpbc.previousRequest),this.additionalRequest||(c.data.page_number=1),/* Prevent reloading on the playlist edit as the playlist videos are one request and library videos another */
"existingPlaylists"!==this.mediaCollectionViewType&&(wpbc.previousRequest=e),this.videoIds&&(c.data.videoIds=this.videoIds.length?this.videoIds:"none"),c.data.query=void 0,_.contains(["libraryPlaylists","existingPlaylists"],this.mediaCollectionViewType)||this.killPendingRequests();var f=this.checksum(c.data);if(!_.isUndefined(wpbc.responses[f]))return this.parse({data:wpbc.responses[f]},"cached"),!0;var g=$.ajax({type:"POST",url:wp.ajax.settings.url,context:this,data:c.data}).done(function(a,b,c){this.parse(a,b,c,f)}).fail(this.fetchFail);return wpbc.requests.push(g),wpbc.broadcast.trigger("spinner:on"),g}/**
				 * Call wp.media.model.MediaCollection.sync or Backbone.sync
				 */
return d=MediaCollection.prototype.sync?MediaCollection.prototype:Backbone,d.sync.apply(this,arguments)},fetchFail:function(){this.pageNumber>1&&this.pageNumber--,wpbc.broadcast.trigger("fetch:finished"),wpbc.broadcast.trigger("spinner:off"),wpbc.broadcast.trigger("fetch:apiError"),status},/**
...
this.startGridView()}),this.listenTo(wpbc.broadcast,"tabChange",function(a){this.model.set(a),this.detailsView instanceof MediaDetailsView&&(this.detailsView.remove(),this.detailsView=void 0),this.render()}),this.listenTo(wpbc.broadcast,"edit:media",function(a){if("videos"===this.model.get("mediaType")){
// We just hit the edit button with the edit window already open.
if("editVideo"===this.model.get("mode"))return!0;this.$el.find(".brightcove-message").addClass("hidden"),this.editView=new VideoEditView({model:a}),this.registerSubview(this.editView),this.model.set("mode","editVideo"),this.render()}else{
// We just hit the edit button with the edit window already open.
if("editPlaylist"===this.model.get("mode"))return!0;this.editView=new PlaylistEditView({model:a}),this.registerSubview(this.editView),this.model.set("mode","editPlaylist"),this.render()}}),this.listenTo(wpbc.broadcast,"preview:media",function(a){if("videos"===this.model.get("mediaType")){
...
wpbc.broadcast.trigger("back:editvideo",a)},insertIntoPost:function(a){a.preventDefault(),
...
var a=wpbc.preload.plupload.drop_element.replace(/[^a-zA-Z0-9-]+/g,"");$("#"+a).on("dropzone:leave",_.bind(this.hide,this))},uploaderAfterInit:function(a){var b,c,d,e=wpbc.preload.plupload.drop_element.replace(/[^a-zA-Z0-9-]+/g,""),f=$("#"+e);
...
b.val(JSON.stringify(f))},/**
		 ...
		 * @returns {boolean}
		 */
removeAttachment:function(a){var b=$(a.currentTarget).parents(".attachment"),c=b.find(".-image");
// Empty the field
b.next("input").val(""),
...
// Custom fields
var e={},f=this.model.get("custom");_.each(this.$el.find(".brightcove-custom-string, .brightcove-custom-enum"),function(a){var b=a.getAttribute("data-id"),c=a.value.trim();if(""!==c){e[b]=c;_.find(f,function(a){return a.id==b}).value=c}}),this.model.set("custom_fields",e),this.model.set("custom",f),this.model.save().done(function(){if(b.length>0){
...
_.each(c,function(a){""!==(a=a.trim())&&wpbc.preload.tags.push(a)}),wpbc.preload.tags.sort()}}).always(function(){
...
if("existingPlaylists"!==this.collection.mediaCollectionViewType){// How many px from bottom until we fetch the next page.
...
this.syncPlaylist()},syncPlaylist:function(){wpbc.broadcast.trigger("playlist:changed",this.videoIds)}}),App={renderMediaManager:function(a){var b=$(".brightcove-media-"+a);document.getElementById("content_ifr");if(b.length){new BrightcoveMediaManagerView({el:b,date:"all",embedType:"page",preload:!0,mode:"manager",search:"",accounts:"all",tags:"all",mediaType:a,viewType:"grid"}).render()}},load:function(){wpbc.requests=[],wpbc.responses={},wpbc.broadcast=_.extend({},Backbone.Events),// pubSub object
this.loaded()},loaded:function(){var a=$(".brightcove-modal"),b=new BrightcoveRouter;wpbc.triggerModal=function(){wpbc.modal?wpbc.modal.$el.show():(wpbc.modal=new BrightcoveModalView({el:a,tab:"videos"}),wpbc.modal.render(),wpbc.modal.$el.find(".spinner").addClass("is-active")),
...
_.each(["videos","playlists"],function(a){App.renderMediaManager(a)}),$(".account-toggle-button").on("click",function(a){a.preventDefault(),$(this).hide(),$(".brightcove-account-row.hidden").show()}),$(".brightcove-add-new-video").on("click",function(a){a.preventDefault(),b.navigate("add-new-brightcove-video",{trigger:!0})}),$(document).on("click",".brightcove-add-media",function(a){a.preventDefault(),wpbc.triggerModal(),wpbc.modal.target=a.currentTarget.dataset.target}),$(document).keyup(function(a){27===a.keyCode&&
// Close modal on ESCAPE if it's open.
wpbc.broadcast.trigger("close:modal")}),$("a.brightcove-action-delete-source").on("click",function(a){var b=$(this).data("alert-message");if(!confirm(b))return!1})}};jQuery(document).ready(function(){App.load();new BrightcoveRouter;Backbone.history.start()});
} )( jQuery );